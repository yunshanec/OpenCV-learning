...
def find_bot_circle_point(image_path):
    image = cv2.imread(image_path)
    image_gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    ret, thresh = cv2.threshold(image_gray, 20, 255, cv2.THRESH_BINARY_INV)
...
if __name__ == "__main__":
    ls_points = []
    for i in range(1, 10):
        point = find_bot_circle_point("../bot_camera/{}.bmp".format(i))
        print(point)
